...
use qcrypto::{
    QuantumChannel,
    protocols::{b92, bb84, bbm92, e91, qia_qzkp, sarg04, six_state},
};
use rayon::prelude::*;
...
mod tests {
    ...
    fn zero_ch() -> QuantumChannel {
        let mix = single_channel_mix(&crate::channel::ChannelKind::BitFlip, 0.0, 0.0);
    ...
    fn run_bbm92_sequential() {
        ...
        let ch = zero_ch();
        let rd = run_bbm92(0, 100, &ch, &ch, noop_info(), 0.0, 0.5, false, false);
        assert!(matches!(rd, RunData::Qkd(_)));
    }
    ...
    fn run_e91_sequential() {
        ...
        let ch = zero_ch();
        let rd = run_e91(0, 100, &ch, &ch, noop_info(), 0.0, 0.5, false, false);
        assert!(matches!(rd, RunData::Qkd(_)));
    }
